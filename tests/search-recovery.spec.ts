...
test('automatic recovery fills a gap and preserves requirements and search history after reload', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('textbox', { name: 'What do you need?' }).fill('recovery scenario under $1000');
  ...
  await expect(page.getByRole('button', { name: 'Prepare shopping links' })).toBeEnabled();
});

test('exhaustion offers explicit requirement review, while an outage only offers retry', async ({ page }, testInfo) => {
  await page.goto('/');
  await page.getByRole('textbox', { name: 'What do you need?' }).fill('recovery scenario no match under $1000');
  await page.getByRole('button', { name: 'Make my list' }).click();
  await page.getByRole('button', { name: 'Find products for this list' }).click();
  const recovery = page.getByRole('region', { name: 'Automatic search recovery' });
  await expect(recovery).toContainText('Nothing has been changed automatically');
  await expect(page.getByRole('button', { name: 'Prepare shopping links' })).toBeDisabled();
  ...
  await expect(page.getByRole('textbox', { name: 'What do you need?' })).toHaveValue('recovery scenario no match under $1000');
  await page.getByRole('textbox', { name: 'What do you need?' }).fill('recovery scenario outage under $1000');
