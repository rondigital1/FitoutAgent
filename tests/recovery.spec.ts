...
test('prompt → list → selection → every shopping link → reload → edit prompt', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'What do you need?' })).toHaveValue('Set up an office for two under $1500');
  await page.getByRole('textbox', { name: 'What do you need?' }).fill('Camping for two under $1000');
...
test('missing results remain visible and editable; provider errors are shown', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Edit list and budget' }).click();
...
test('older streamed state without sources still renders the prompt', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'What do you need?' })).toBeVisible();
