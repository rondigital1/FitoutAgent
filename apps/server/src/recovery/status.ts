import type { ChecklistItem, Offer, Requirements, SearchRecovery, State } from '@settlein/shared';
import { eligible } from '../optimizer';

export function suitable(offer: Offer, r: Requirements) {
  return eligible(offer, r) && offer.price !== null && (offer.fit?.status === 'verified' || offer.source === 'Mock');
}
...
export function finishRecovery(s: State, item: ChecklistItem, record: SearchRecovery) {
  ...
  } else if (candidates.some(o => eligible(o, s.requirements!))) {
