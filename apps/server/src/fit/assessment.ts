import { z } from 'zod';
import type { Offer, ProductFit } from '@settlein/shared';
...
export function unknownFit(summary = 'Product fit could not be verified. Review the product specifications.'): ProductFit {
  return { status: 'unknown', checks: [], summary, assessedAt: new Date().toISOString() };
}
